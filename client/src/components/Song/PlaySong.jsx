...
//         const likeResponse = await api.get(`/like/${id}/`, {
//           headers: {
//             Authorization: `Token ${token}`,
//           },
//         });
...
//       if (liked) {
//         // Deleting the like
//         await api.delete(`/likes/${id}/delete/`, {
...
//           song_id: song.id, // send song_id as part of the request body
//           },
//           { 
...
//       }
//     } catch (error) {
//       console.error(liked ? 'Error unliking:' : 'Error liking:', error);
//     }
//   };
...
//               <button
//                 className="bg-blue-600 text-white p-4 rounded-full shadow-lg hover:bg-blue-700 hover:shadow-2xl transition duration-300"
//               >
...
//             {like && like.length > 0 ? (
//               like.map((like) => (
//                 <div key={like.id} className="flex items-center w-full mt-2">
//                   <div className="flex-1">
//                     <p className="text-gray-300 text-sm">
//                       <span className="font-semibold">{like.user?.username}</span> liked this song.
//                     </p>
//                   </div>
//                 </div>
//               ))
...
//                 <button
//                   type="submit"
//                   className="text-green-400 p-2 rounded-full transition duration-300"
//                 >
//                   <FaComment size={24} />
//                 </button>
...
//                   <button
//                     onClick={browseSongs}
//                     className="text-cyan-200 text-2xl p-0"
//                   >
...
export default function SongDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const likeResponse = await api.get(`/songs/${id}/likes/`, {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
  ...
  const handleLike = async (e) => {
    ...
    try {
      if (liked) {
        await api.delete(`/songs/${id}/like/delete/`, {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
